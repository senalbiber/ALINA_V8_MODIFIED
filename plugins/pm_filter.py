...
async def advantage_spoll_choker(bot, query):
    ...
    if not movies:
        return await query.answer("Button එක expire වෙලා ආයෙ නම ගහලා දාන්න......Button expired and again please send the film name", show_alert=True)
    movie = movies[(int(movie_))]
    await query.answer('බලමු👀 database එකෙ තියෙනවද කියලා..Let see if it is in my database...')
    ...
    if k == False:        
        ...
        if files:
            ...
            k = await query.message.edit('Sorry, ඕක අපි ගාව නෑ @admin ගහලා film එකේ නම එවන්න🙂....Sorry bro, we do not have it.please send like this👉 ( @admin spider man )... ')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if f_caption is None:
            ...
            buttons = [
        [
             InlineKeyboardButton('🛡 Join Group 🛡', url='t.me/cinamahublk') 
        ]
        ]
            reply_markup=InlineKeyboardMarkup(buttons)
        await query.answer()
...
async def advantage_spell_chok(msg):
    ...
    await msg.reply("ඔය කියන විදියේ එකක් නම් නෑ.😕 ඔයාගේ spellings වැරදියි! බලන්න මෙතන තියෙනවද කියල හොයන එක??👇.....There is no such thing as you say. Your spellings are wrong!😕 See if you can find it here ????", reply_markup=InlineKeyboardMarkup(btn))
